...
![image-20220708191907185](D:\Cpp\Linux网络编程笔记\image\image-20220708191907185.png)
...
对套接字进行操作的fcntl()有4种;![image-20220710155015045](D:\Cpp\Linux网络编程笔记\image\image-20220710155015045.png)
...
![image-20220710163313083](D:\Cpp\Linux网络编程笔记\image\image-20220710163313083.png)
...
![image-20220710171017345](D:\Cpp\Linux网络编程笔记\image\image-20220710171017345.png)
...
这样做需要本地主机的带宽与目标主机之间带宽进行比拼。可以采用多线程的方法一次性地发送多个ICMP请求报文,让目标主机处理过程出现问题而速度缓慢或者宕机。直接攻击的方法有一个缺点,就是可以根据来源的IP地址屏蔽攻击源,并且目标源容易暴露,被对方反攻击。攻击的过程如图13.16![image-20220710185210796](D:\Cpp\Linux网络编程笔记\image\image-20220710185210796.png)

##### 伪装IP攻击:

在直接洪水攻击的基础上,将发送方的IP地址用伪装的IP地址代替。将直接洪水攻击的缺点进行了改进。攻击的过程如图13.17所示。

 ![image-20220710185318725](D:\Cpp\Linux网络编程笔记\image\image-20220710185318725.png)

##### 反射攻击:

不是直接对目标机进行攻击,而是让其他一群主机误认为目标机在向它们发送ICMP请求包,一群主机向目标机发送ICMP应答包。攻击方向一群主机发送ICMP请求,将请求的源地址伪装成目标机的IP地址,这样目标机就成了ICMP回显反射的焦点,如图13.18所示。攻击方的IP地址为10.10.10.10,向一组服务器a、b、.等发送ICMP回显请求报文,并将发送方的IP地址伪装成目标机的IP地址10.10.8.10。服务器组在经过处理后,会发送ICMP的回显报文给目标机。![image-20220710185758828](D:\Cpp\Linux网络编程笔记\image\image-20220710185758828.png)
...
![image-20220711103150311](D:\Cpp\Linux网络编程笔记\image\image-20220711103150311.png)
...
![image-20220711133853078](D:\Cpp\Linux网络编程笔记\image\image-20220711133853078.png)
...
​	在客户端连接到来的时候,将客户端的连接放到一个状态池中,对所有客户端的连接状态在一个处理单元中进行轮询处理。与前面的并发服务器相比,客户端的增加不会造成系统并行处理单元的增加,而处理能力与CPU和内存的速度直接相关。![image-20220711141540291](D:\Cpp\Linux网络编程笔记\image\image-20220711141540291.png)
...
![image-20220711155042639](D:\Cpp\Linux网络编程笔记\image\image-20220711155042639.png)
...
![image-20220711155719278](D:\Cpp\Linux网络编程笔记\image\image-20220711155719278.png)

## IPv6 的UDP头部

<img src="D:\Cpp\Linux网络编程笔记\image\image-20220711160149066.png" alt="image-20220711160149066" style="zoom:150%;" />

## IPv6 的ICMP 头部

![image-20220711160307925](D:\Cpp\Linux网络编程笔记\image\image-20220711160307925.png)

## IPv6 的套接字选项

### setsockopt()  

![image-20220711163639971](D:\Cpp\Linux网络编程笔记\image\image-20220711163639971.png)

![image-20220711163652054](D:\Cpp\Linux网络编程笔记\image\image-20220711163652054.png)
...
![image-20220712100725263](D:\Cpp\Linux网络编程笔记\image\image-20220712100725263.png)

![image-20220712100802481](D:\Cpp\Linux网络编程笔记\image\image-20220712100802481.png)

网络数据在内核中的处理过程主要是在网卡和协议栈之间进行:从网卡接收数据,交给协议栈处理;协议栈将需要发送的数据通过网络发出去。

![image-20220712101026824](D:\Cpp\Linux网络编程笔记\image\image-20220712101026824.png)
...
​	接收数据的过程，当有数据到来的时候,网卡的中断处理程序将数据从以太网网卡的FIFO对列中接收到内核,传递给协议层,协议层在IP层剥离IP的首部、UDP层剥离UDP的首部、TCP层剥离TCP的首部后传递给插口层,插口层查询socket的标识后,将数据送给用户层匹配的socket。![image-20220712101749368](D:\Cpp\Linux网络编程笔记\image\image-20220712101749368.png)

​	最上面是用户空间层,应用层的程序位于此处。最底部是物理设备,例如以太网网卡等,提供网络数据的连接、收发。中间是内核层,即网络协议栈子系统。流经网络栈内部的是socket缓冲区(由结构sk_buffs接连),它负责在源和汇点之间传递报文数据。

​	顶部是系统调用接口,它为用户空间的应用程序提供了一种访问内核网络子系统的接口。位于其下面的是一个协议无关层,它提供了一种通用方法来使用底层传输层协议。然后是实际协议,在Linux中包括内嵌的协议TCP、 UDP,当然还有IP。然后是另外一个网络设备协议无关层,提供了与各个设备驱动程序通信的通用接口,最下面是设备驱动程序本身。![image-20220712102820850](D:\Cpp\Linux网络编程笔记\image\image-20220712102820850.png)
...
![image-20220712110124368](D:\Cpp\Linux网络编程笔记\image\image-20220712110124368.png)
...
![image-20220712112120953](D:\Cpp\Linux网络编程笔记\image\image-20220712112120953.png)
...
![image-20220712161025773](D:\Cpp\Linux网络编程笔记\image\image-20220712161025773.png)

## SHTTPD的模块分析和设计

### SHTTPD 的模型框架

![image-20220712161517896](D:\Cpp\Linux网络编程笔记\image\image-20220712161517896.png)
...
![image-20220712163247853](D:\Cpp\Linux网络编程笔记\image\image-20220712163247853.png)
...
![image-20220712163856011](D:\Cpp\Linux网络编程笔记\image\image-20220712163856011.png)

![image-20220712164115122](D:\Cpp\Linux网络编程笔记\image\image-20220712164115122.png)
...
![image-20220712190051530](D:\Cpp\Linux网络编程笔记\image\image-20220712190051530.png)

采用进程间的管道通信方式,来获得CGI程序的输出并发送到客户端。CGI执行程序的输出为标准输出,为了在主进程中能够获得CGI执行进程的CGI输出,这里采用了进程间的管道通信方式并使用了文件描述符的复制操作,将CGI执行进程中管道的一端与标准输出绑定起来,CGI程序的输出数据会进行管道,主程序可以在另一端接收到CGI执行结果在标准输出的结果。![image-20220712190410770](D:\Cpp\Linux网络编程笔记\image\image-20220712190410770.png)

![image-20220712190454861](D:\Cpp\Linux网络编程笔记\image\image-20220712190454861.png)
